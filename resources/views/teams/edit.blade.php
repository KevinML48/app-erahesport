        ...
        <div class="login-form">
            ...
            <div class="form-inputs">
                <form action="{{ route('teams.update', $team->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="input-box">
                        <input type="text" class="input-field" name="name" placeholder="Nom de l'équipe" value="{{ $team->name }}" required>
                    </div>

                    <!-- Champ pour la description de l'équipe -->
                    <div class="input-box">
                        <textarea class="input-field" name="description" placeholder="Description de l'équipe">{{ $team->description }}</textarea>
                    </div>

                    <!-- Champ pour l'image de l'équipe -->
                    <div class="input-box file-input-wrapper">
                        <label for="image">
                            <img id="imagePreview" src="{{ asset('storage/' . $team->image) }}" alt="Aperçu de l'image">
                        </label>
                    ...
                    <div class="input-box">
                        <select name="domain_id" class="input-field">
                            @foreach($domains as $domain)
                                <option value="{{ $domain->id }}" {{ $team->domain_id == $domain->id ? 'selected' : '' }}>{{ $domain->name }}</option>
                            @endforeach
                        </select>
                    </div>
